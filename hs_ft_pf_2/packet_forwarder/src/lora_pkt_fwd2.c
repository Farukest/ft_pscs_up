...
static int parse_gateway_configuration(const char * conf_file) {
    ...
    str = json_object_get_string(conf_obj, "collector_address");
    if (str != NULL) {
        strncpy(collector_addr, str, sizeof collector_addr);
        collector_addr[sizeof collector_addr - 1] = '\0'; /* ensure string termination */
